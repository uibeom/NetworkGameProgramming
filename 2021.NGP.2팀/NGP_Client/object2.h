...
float O2_pointx = 2.25f;
float O2_pointz = 0.0f;
float O2_pointy = 0.0f;

glm::vec3 Object2Coord(2.25f, 0.0f, 0.0f);

float O2_setat = 0.0f;      //이동시 다리가 움직이는 애니메이션을 하기위한 각도
float O2_setam = 180.0f;      //몸통 회전을 맡음
float O2_setaRighthand = 60.0f;
float O2_setaLefthand = 60.0f;
...
void drawObject2(GLuint program, int vertexCount, GLuint vao[], GLuint vbo[], glm::mat4 viewMatrix, glm::mat4 projectionMatrix) {
    ...
    transMatrixpoint = glm::translate(transMatrixpoint, glm::vec3(Object2Coord.x, Object2Coord.y, Object2Coord.z));
    rotateMatrixm = glm::rotate(rotateMatrixm, glm::radians(O2_setam), glm::vec3(0.0f, 1.0f, 0.0f));      //몸통 회전
