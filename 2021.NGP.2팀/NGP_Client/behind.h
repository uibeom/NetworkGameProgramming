...
float B_setat = 0.0f;      //이동시 다리가 움직이는 애니메이션을 하기위한 각도
...
void drawBehind(GLuint program, int vertexCount, GLuint vao[], GLuint vbo[], glm::mat4 viewMatrix, glm::mat4 projectionMatrix) {
    ...
    transMatrixpoint = glm::translate(transMatrixpoint, glm::vec3(BehindCoord.x, BehindCoord.y, BehindCoord.z));
    rotateMatrixm = glm::rotate(rotateMatrixm, glm::radians(B_setam), glm::vec3(0.0f, 1.0f, 0.0f));      //몸통 회전
    ...
    objColor = glm::vec3(1.0f, 0.0f, 1.0f);
    ...
    transformMatrix = basicChange * transformMatrix;
    basicChange = glm::mat4(1.0f);      //초기화
    basicChange = glm::scale(basicChange, glm::vec3(1.0f, 1.7f, 6.0f));      //사이즈 조절
    transformMatrix = basicChange * transformMatrix;


    basicChange = glm::mat4(1.0f);
    basicChange = glm::translate(basicChange, glm::vec3(-9.0, 0.8f, 0.0));      //바닥의 윗면으로 이동
    transformMatrix = basicChange * transformMatrix;
    transformMatrix = rotateMatrixm * transformMatrix;      //몸통의 회전
    transformMatrix = transMatrixpoint * transformMatrix;      //몸통의 이동
    transformMatrix = charTerrace * transformMatrix;      //난간위로 이동
